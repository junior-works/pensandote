...
-- ---------------------------------------------------------------------
-- Tablas: pensamientos, fotos_dia, fechas_afectivas, contactos_ultimo,
--         historias, historia_visibilidad, historia_interacciones.
-- Columna: circles.ntfy_topic (con default generado + backfill).
-- Storage: buckets fotos, historias (privados, RLS por círculo).
--
...
-- =====================================================================
-- Helper: ¿es "hijo/a" del narrador en este círculo?
-- ---------------------------------------------------------------------
-- DECISIÓN: matcheamos `parentesco` con `^hij[oa]\b` (case-insensitive)
-- en vez de agregar una columna `es_hijo bool`. Razón: el parentesco ya
-- lo escribe el admin, lo entiende el usuario, y si el regex falla en
-- algún caso ("Hijastra", "Hija mayor") podemos extenderlo acá sin
-- migración. Si esto resulta frágil con datos reales, agregamos la
-- columna en una 0004 y reescribimos este helper.
-- =====================================================================
...
-- =====================================================================
-- TABLA: contactos_ultimo (upsert por (circle_id, con_user_id))
-- =====================================================================
...
-- =====================================================================
-- TABLA: historia_visibilidad (solo se usa cuando visibilidad='especificas')
-- =====================================================================
...
-- SELECT: si yo puedo SELECT la historia (RLS de historias filtra), veo la fila.
...
-- WRITE: sólo el narrador de la historia puede modificar su visibilidad.
...
-- =====================================================================
-- TABLA: historia_interacciones (repreguntas + favorita)
-- =====================================================================
...
-- SELECT: si yo puedo SELECT la historia, veo sus interacciones.
create policy hi_select on public.historia_interacciones for select using (
    exists (select 1 from public.historias h where h.id = historia_id)
);

-- INSERT: cualquiera con acceso a la historia, escribiendo en su nombre.
...
-- UPDATE/DELETE: sólo el autor de la interacción.
